"""
CAPI System Instruction Generator - OPTIMIZED VERSION.
Creates YAML configuration for Conversational Analytics API with async I/O.

KEY OPTIMIZATIONS:
- Async file I/O (non-blocking writes)
- Async summary report generation

PERFORMANCE:
- Before: Single blocking file write
- After: Async file I/O allows other operations to continue
- Expected: Minor improvement (~0.5-1s saved on file I/O)
...
cohesive document. The optimization here is primarily in I/O operations.
"""
import logging
import os
import json
...
class CAPIInstructionGeneratorOptimized:
    """OPTIMIZED: Agent for generating Conversational Analytics API system instructions."""

    def __init__(self):
        self.client = get_claude_vertex_client()  # Claude 4.5 for complex YAML generation
        logger.info("CAPI Instruction Generator OPTIMIZED initialized")

    async def execute(self, state: Dict) -> Dict:
        """Execute CAPI system instruction generation phase with async I/O."""
        import time

        logger.info("🚀 Generating CAPI system instructions (YAML) - OPTIMIZED")
        ...
        try:
            ...
            if "job_manager" in state and "job_id" in state:
                ...
                job_manager.add_log(
                    job_id,
                    "capi instruction generator optimized",
                    f"📝 Generating CAPI YAML system instructions...",
                    "INFO"
                )
